...
module.exports = (log, config) => {
  ...
  const OAuthAPI = createBackendServiceAPI(log, config, 'oauth', {
    ...
    grantTokensFromAssertion: require('./grant-tokens-from-assertion')(config),
  })
  ...
  return {
    ...
    async grantTokensFromSesionToken(sessionToken, oauthParams) {
      oauthParams.assertion = await makeAssertionJWT(config, sessionToken)
      try {
        return await api.grantTokensFromAssertion(oauthParams)
      } catch (err) {
        throw mapOAuthError(log, err)
      }
    },
