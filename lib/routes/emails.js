      ...
      options: {
        validate: {
          payload: {
            ...
            service: validators.service,
            reminder: isA.string().regex(/^(?:first|second)$/).optional(),
          ...
          .then((account) => {
            ...
            return db.deviceFromTokenVerificationId(uid, code)
              ...
              .catch(err => {
                ...
                log.error('account.signin.confirm.invalid', { uid, code, err })
                throw err
              })
              .then(() => {
                if (device) {
                ...
                return db.verifyEmail(account, account.emailCode)
                  .then(() => {
                    return P.all([
                      ...
                      reminder ? request.emitMetricsEvent('account.reminder', { uid }) : null,
