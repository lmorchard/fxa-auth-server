...
module.exports = (log, config, oauthdb) => {
  const routes = [
    ...
    },
    {
      ...
      options: {
        auth: {
          // XXX TODO: This route must also be able to accept 'client_secret' as Basic Auth in header.
          mode: 'optional',
          strategy: 'sessionToken'
        },
        validate: {
          payload: Joi.alternatives.try(
            ...
            oauthdb.api.grantTokensFromRefreshToken.validate.payload,
            oauthdb.api.grantTokensFromAssertion.opts.validate.payload.keys({
              assertion: Joi.forbidden()
            })
          )
        },
        response: {
          schema: Joi.alternatives.try(
            ...
            oauthdb.api.grantTokensFromRefreshToken.validate.response,
            oauthdb.api.grantTokensFromAssertion.opts.validate.response
          )
        }
      },
      handler: async function (request) {
